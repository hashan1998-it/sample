...
export interface FormItemProps extends CommonProps {
    ...
    // New props for error message customization
    errorMessageSize?: 'xs' | 'sm' | 'md' | 'lg'
    errorMessageResponsive?: boolean
}
...
const FormItem = (props: FormItemProps) => {
    ...
    const getErrorMessageSizeClass = () => {
        const sizeClasses = {
            xs: 'text-xs',
            sm: 'text-sm',
            md: 'text-base',
        ...
        return errorMessageResponsive 
            ? responsiveClasses[errorMessageSize]
            : sizeClasses[errorMessageSize]
    }
    ...
    const enterStyle = { opacity: 1, marginTop: 4, bottom: -21 }
    const exitStyle = { opacity: 0, marginTop: -8 }
    const initialStyle = exitStyle

    return (
        <FormItemContextProvider value={{ invalid: !!invalid }}>
