...
const PostLoginLayout = ({ layoutType, children }: PostLoginLayoutProps) => {
    const AppLayout = layouts[layoutType as keyof typeof layouts] ?? layouts[Object.keys(layouts)[0] as keyof typeof layouts]

    if (!AppLayout) {
        return (
            <div className="flex flex-auto flex-col h-[100vh]">
                <Loading loading={true} />
                <div className="text-center mt-4 text-red-500">Layout not found.</div>
            </div>
        )
    }
